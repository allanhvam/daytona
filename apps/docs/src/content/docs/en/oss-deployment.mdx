...
**Credentials**: UniFi requires three values:

- `UNIFI_API_KEY`: API key from UniFi Network Console > Integrations
- `UNIFI_BASE_URL`: Base URL of the UniFi Network API endpoint, for example `https://your-unifi-controller/proxy/network/integration/v1`
- `UNIFI_SITE_ID`: UniFi site UUID (you can fetch this from the `/sites` endpoint)

**Caddy module**: `github.com/caddy-dns/unifi`
